(ns shirohan.plate
  ...
  ## choke（縮み代）—— **縮小ではなく、外側を削る**

  白インクと色インクの刷りは**若干ずれる**。白版が色版とぴったり同じ大きさだと、
  ずれた瞬間に白が図案の外へはみ出して縁が白く光る（白フチ／ハロー）。そこで
  白版は図案より **0.1mm 削る**。

  **「0.1mm 縮小」ではない。** 相似縮小と侵食は、穴のある形で結果が逆になる:

  ```
  縮小（相似変換）              侵食（オフセット）＝正しい
  ┌────────────────┐          ┌────────────────┐
  │  ███████████   │          │  ███████████   │
  │  ██┌──────┐██  │          │  ██┌────────┐█ │
  │  ██│ 穴も │██  │          │  ██│ 穴は   │█ │
  ...
  │  ██└──────┘██  │          │  ██└────────┘█ │
  └────────────────┘          └────────────────┘
   → 穴に白版がはみ出す         → 穴の縁でも白が引っ込む
  ```

  ドーナツ型で単純に縮小すると、**ドーナツの穴の部分に白版が出てくる**
  （実務家の指摘、2026-08-01）。目的は「インクが載る面を全周から choke だけ
  削る」ことなので、外周は内へ、**穴は外へ**動かす。

  `shirohan.geom/offset` は「δ>0 で囲む面が広がる」符号なので、輪郭が
  インクの外周か穴の縁かを入れ子の深さの偶奇で判定し、外周に `-choke`、
  穴に `+choke` を渡す。回帰試験は `plate_test` の
  `choke-erodes-the-ink-face-it-does-not-scale-the-shape`（外周 −1mm /
  穴 +1mm を実寸で固定）。
