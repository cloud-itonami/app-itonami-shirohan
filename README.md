...
図案の **外側を 0.1mm 削る**（`:choke-mm`、実務値）。

**「0.1mm 縮小」ではない。** 相似縮小と侵食は、穴のある形で結果が逆になる:

| | 外周 | 穴 | 結果 |
|---|---|---|---|
| 縮小（相似変換） | 内へ | **内へ**（穴が縮む） | ドーナツの穴に白版がはみ出す |
| 侵食（オフセット）＝正しい | 内へ | **外へ**（穴が広がる） | 穴の縁でも白が引っ込む |

```
図案の輪郭 ─┐
            ├─ 0.1mm 内側 ──▶ 白版
穴の縁     ─┴─ 0.1mm 外側 ──▶ 白版（穴が広がる＝白が引っ込む）
```

**穴が逆向きに動く**のが要点。目的は「インクが載る面を全周から削る」ことなので、
外周は内へ、穴は外へ動かす。どちらの輪郭かは**入れ子の深さの偶奇**で決める
（「O」の字は外周と内周の2本でできていて、内周が囲んでいるのは穴）。

回帰試験 `choke-erodes-the-ink-face-it-does-not-scale-the-shape` が、外周 r=80mm /
穴 r=35mm のドーナツに choke 1mm をかけて **外周 79mm・穴 36mm** になることを実寸で
固定している。穴が 34mm 側へ動いたら相似縮小になっている。
